...
Future<void> _showAddTaskDialog() async {
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 3, offset: const Offset(2, 2))],
                ),
                child: Row(
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(task['title'], style: const TextStyle(fontFamily: 'HappyMonkey', fontSize: 16, color: Colors.black)),
                          if (isQuest && task.containsKey('xp'))
                            Text('${task['xp']} XP', style: TextStyle(fontSize: 12, color: Colors.indigo)),
                        ],
                      ),
                    ),
                    GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        child: AnimatedOpacity(opacity: isChecked ? 1 : 0, duration: const Duration(milliseconds: 200), child: const Icon(Icons.check, size: 16, color: Colors.black)),
                      ),
                    ),
                  ],
                ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        title: Text('Home', style: TextStyle(fontFamily: 'HappyMonkey', fontSize: Theme.of(context).textTheme.bodyMedium?.fontSize, color: Theme.of(context).textTheme.bodyMedium?.color)),
      ),
      body: ListView(
        ...
        children: [
          _buildTaskList(),
          const SizedBox(height: 20),
          Center(child: InkWell(onTap: _showAddTaskDialog, child: Material(elevation: 3, shape: const CircleBorder(side: BorderSide(color: Colors.black)), child: Container(width: 50, height: 50, decoration: const BoxDecoration(shape: BoxShape.circle, color: Colors.white), child: const Icon(Icons.add, size: 30, color: Colors.black87))))),
          const SizedBox(height: 20),
          Center(child: Column(children: const [Text('Your journey to a\nbetter you starts here.', style: TextStyle(fontSize: 16, fontFamily: 'HappyMonkey'), textAlign: TextAlign.center), SizedBox(height: 8), Text('Be yourself. Thrive.', style: TextStyle(fontSize: 16, fontFamily: 'HappyMonkey'), textAlign: TextAlign.center), SizedBox(height: 8), Text('Find Peace in the\nPresent Moment.', style: TextStyle(fontSize: 16, fontFamily: 'HappyMonkey'), textAlign: TextAlign.center)])),
        ],
      ),
      bottomNavigationBar: Container(
        padding: const EdgeInsets.symmetric(vertical: 8),
        decoration: BoxDecoration(color: Theme.of(context).colorScheme.surface, boxShadow: const [BoxShadow(color: Colors.black12, blurRadius: 4, offset: Offset(0, -2))]),
  ...
  Widget _navItem(IconData icon, int idx, BuildContext context) {
    ...
    final fill = isSel ? (Theme.of(context).brightness == Brightness.dark ? const Color(0xFF40D404) : const Color(0xFFB6FFB1)) : Theme.of(context).colorScheme.surface;
    final col = isSel ? Colors.black : Colors.grey[700];
    return Material(elevation: 3, shape: const CircleBorder(side: BorderSide(color: Colors.black)), child: InkWell(borderRadius: BorderRadius.circular(25), onTap: () {
      setState(() => _selectedIndex = idx);
      if (idx == 0) Navigator.pushReplacement(context, createFadeRoute(const SettingsScreen()));
      if (idx == 1) Navigator.pushReplacement(context, createFadeRoute(const InsightsScreen()));
      if (idx == 2) Navigator.pushReplacement(context, createFadeRoute(const HomeScreen()));
      if (idx == 3) Navigator.pushReplacement(context, createFadeRoute(const ActivitiesScreen()));
      if (idx == 4) Navigator.pushReplacement(context, createFadeRoute(const ProfileScreen()));
    }, child: Container(width: 50, height: 50, decoration: BoxDecoration(shape: BoxShape.circle, color: fill), child: Icon(icon, color: col))),
    );
  }
}